...
Enunciato secondo \textbf{Caratheodory}: 

\textbf{Tipi di cicli}
Ciclo di \textbf{Otto, }:
Ciclo \textbf{Diesel}:
Ciclo di \textbf{Carnot}:

\textbf{Trasformazione termodinamica reversibile} \\
Una trasformazione termodinamica reversibile è una successione di trasformazioni infinitesime che passa istante per istante per degli stati di equilibrio e tale che esiste una trasformazione che riporta il sistema e anche l'esterno nello stato iniziale.
In pratica si può disegnare sul piano $(V,p)$.

\textbf{Entropia} \\
Come conseguenza del secondo principio si ha che $$ _R \oint \frac{\delta Q}{T}=0$$. \\
...
Quindi si può definire una funzione di stato $S$ tale che per ogni coppia di stati $A$ e $B$ valga $$S(B)-S(A)= _R \int_{A\rightarrow B} \frac{\delta Q}{T}$$
...
\textbf{Rendimento di un ciclo} \\
Il rendimento si calcola di un ciclo che effettua degli scambi di calore tra due sorgenti, per esempio per un ciclo di Carnot o un ciclo Stirling.
Inserire i due disegni. 
